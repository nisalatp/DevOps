...
EOF
  ...
  # --- Pre-install PKI certificates if using manual distribution ---
  # If the join command contains --certificate-key, kubeadm will download
  # the certs automatically from the first CP. But if no certificate key
  # is available (e.g., upload-certs was skipped due to the NAT bug),
  # we need to manually copy the shared PKI certificates first.
  #
  # These certs were saved to /vagrant/pki by the first CP's setup script.
  # Only the shared CA and signing keys are needed — node-specific certs
  # (apiserver.crt, etcd/server.crt, etc.) are regenerated by kubeadm
  # for this particular node.
  case "$JOIN" in
    *--certificate-key*)
      info "Join command has --certificate-key — certs will be downloaded automatically."
      ;;
    *)
      if [ -d /vagrant/pki ]; then
        info "No --certificate-key found — pre-installing PKI certs from /vagrant/pki..."
        $SUDO mkdir -p /etc/kubernetes/pki/etcd
        for f in ca.crt ca.key sa.key sa.pub front-proxy-ca.crt front-proxy-ca.key; do
          [ -f "/vagrant/pki/$f" ] && $SUDO cp "/vagrant/pki/$f" "/etc/kubernetes/pki/$f"
        done
        for f in ca.crt ca.key; do
          [ -f "/vagrant/pki/etcd/$f" ] && $SUDO cp "/vagrant/pki/etcd/$f" "/etc/kubernetes/pki/etcd/$f"
        done
        ok "PKI certificates pre-installed from /vagrant/pki."
      else
        warn "No --certificate-key and no /vagrant/pki found."
        warn "The join may fail. Get the cert key from the first CP:"
        warn "  sudo kubeadm init phase upload-certs --upload-certs"
      fi
      ;;
  esac
